...
alink = 'http://py4e-data.dr-chuck.net/geojson?'

while True:
    address = input('Enter location: ')
    if len(address) < 1: 
        break
    url = alink + urllib.parse.urlencode({'address': address})
    print('Retrieving', url)
    urlr = urllib.request.urlopen(url)
    data = urlr.read().decode()
    ...
    if not jfile or 'status' not in jfile or jfile['status'] != 'OK':
    ...
    location = jfile['results'][0]['formatted_address']
    print(location)
